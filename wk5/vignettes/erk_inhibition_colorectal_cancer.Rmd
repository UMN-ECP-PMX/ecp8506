...
```{r}
vp <- readRDS(here("data/s10vpop_pk.RDS")) %>% mutate(VPOP2 = seq(n()))
...
```{r, message = FALSE}
mod <- mread_cache("mapk", here("model"), end  = 56)
...
re_run <- 
  ...
  select(label,object) %>%
  filter(label %in% c("GDC", "COBI+GDC")) %>% 
  mutate(out = future_lapply(object, sim, Vp = vp_select, Mod = mod, future.seed = TRUE)) %>%
  select(label, out) %>% 
  unnest(cols = c(out))
```
