...
# Calculate fraction with Cmax > 15 ug/L ----------------------------------

#' Summrise Cmax by individual
cmax <- out %>% group_by(ID, DOSE) %>% 
  summarise(Cmax=max(IPRED)) %>% ungroup() %>% 
  mutate(Dose=paste0(DOSE, " mg/kg"))

#' Check Cmax distributions by dose
ggplot(cmax)+geom_histogram(aes(x=Cmax))+
  geom_vline(xintercept=15,linetype="dashed",color="red") + # Cmax threshold
  facet_wrap(~Dose)+
  theme_bw()

#' Calculate and plot fraction with Cmin > 15 ug/L
above2 <- cmax %>% group_by(DOSE) %>% 
  summarize(FRAC=fraction_above(Cmax,15))
knitr::kable(above2)

ggplot(data=above2) + 
  geom_line(aes(x=DOSE,y=FRAC),color='red') + 
  geom_hline(yintercept=0.1,linetype="dashed") +
  xlab("Dose (mg/kg)") + ylab("Fraction Cmax > 15 ug/L")+
  theme_bw()
